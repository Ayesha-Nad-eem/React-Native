...
export default function UserTab() {
  ...
  const [fares, setFares] = useState<
    { carId: string; fare: number; breakdown: any; car: any }[]
  >([]);
  ...
  const handlePaymentSuccess = () => {
    ...
    Alert.alert(
      "Booking Confirmed! 🎉",
      `Your booking for ${chosen?.car.modelName} has been confirmed!\n\nWe will contact you at ${customerPhone} with driver details.`
    );
  ...
  async function parseInputToCoords(input: string): Promise<LatLng | null> {
    ...
    try {
      ...
      console.warn(
        "Expo geocode failed, falling back to Google Geocoding:",
        err?.message || err
      );
    ...
    try {
      ...
      console.warn(
        "Google geocode returned no results for:",
        t,
        j.status,
        j.error_message
      );
  ...
  async function onCalculate() {
    ...
    try {
      ...
      if (!fromCoords || !toCoords) {
        Alert.alert(
          "Invalid Input",
          "Please enter valid From and To locations (address or lat,lon)."
        );
        return;
      }
      ...
      if (!apiKey)
        throw new Error(
          "Google Maps API key not found. Please set GOOGLE_MAPS_API_KEY in .env and restart the app."
        );
      ...
      if (json.status !== "OK" || !json.routes || json.routes.length === 0) {
        ...
        throw new Error(
          json.error_message || "No route found via Google Directions"
        );
      }
      ...
      if (mapRef.current && coords && coords.length > 0) {
        mapRef.current.fitToCoordinates(coords, {
          edgePadding: { top: 80, bottom: 200, left: 40, right: 40 },
          animated: true,
        });
      }
      ...
      if (firstRaw) {
        if (typeof firstRaw === "string" && firstRaw.includes('"message":null'))
          opaqueMessageNull = true;
        else if (
          typeof firstRaw === "object" &&
          Object.prototype.hasOwnProperty.call(firstRaw, "message") &&
          firstRaw.message === null
        )
          opaqueMessageNull = true;
      }
      if (opaqueMessageNull) {
        Alert.alert(
          "Toll estimation unavailable",
          "TollGuru returned an opaque error. Please verify your TOLLGURU_KEY in `.env` or your EAS secrets and restart the app."
        );
      }
      setFares(mapped);
    ...
    <View className="flex-1 p-4 bg-white">
      ...
      <View className="mb-4 bg-gray-50 p-3 rounded-lg">
        ...
        <Pressable
          onPress={onCalculate}
          disabled={fetching}
          className={`py-3 rounded-xl ${fetching ? "bg-gray-400" : "bg-blue-600"}`}
        >
          <Text className="text-white text-center font-semibold">
            {fetching ? "Calculating..." : "Show Fares"}
          </Text>
        </Pressable>
      ...
      <Modal
        visible={showFaresModal}
        animationType="slide"
        onRequestClose={() => setShowFaresModal(false)}
      >
        <View className="flex-1 bg-white p-4">
          ...
          <Text className="text-sm text-gray-600 mb-2">
            Distance: {distanceKm ?? "-"} km • Duration: {durationMin ?? "-"}{" "}
            min
          </Text>
          {fares &&
          fares.length > 0 &&
          typeof fares[0].breakdown?.tollFare === "number" ? (
            <Text className="text-sm text-gray-700 mb-3">
              Estimated route tolls: ${fares[0].breakdown.tollFare}
            </Text>
          ) : null}
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity
                  onPress={() => setSelectedCarId(item.carId)}
                  className={`p-3 rounded-lg mb-3 ${selected ? "border-2 border-blue-600 bg-blue-50" : "border border-gray-100 bg-gray-50"}`}
                >
                  {item.car.imageUri ? (
                    <Image
                  ...
                  <Text className="text-gray-700">
                    Capacity: {item.car.capacity} people
                  </Text>
                  <Text className="text-gray-700">
                    Luggage: {item.car.luggageSpace} bags
                  </Text>
                  <View className="mt-2">
                    <Text className="text-sm text-gray-600">
                      Hour charge: $
                      {item.breakdown?.hourFare?.toFixed
                        ? item.breakdown.hourFare.toFixed(2)
                        : (item.breakdown?.hourFare ?? "0.00")}
                    </Text>
                    <Text className="text-sm text-gray-600">
                      Distance charge: $
                      {item.breakdown?.distanceFare?.toFixed
                        ? item.breakdown.distanceFare.toFixed(2)
                        : (item.breakdown?.distanceFare ?? "0.00")}
                    </Text>
                    <Text className="text-sm text-gray-600">
                      Tolls: $
                      {item.breakdown?.tollFare?.toFixed
                        ? item.breakdown.tollFare.toFixed(2)
                        : (item.breakdown?.tollFare ?? "0.00")}
                    </Text>
                    <Text className="mt-1 font-semibold text-gray-800">
                      Total Estimated Fare: ${item.fare}
                    </Text>
                  </View>
                </TouchableOpacity>
              );
      ...
      {selectedCarId && (
        <PaymentModal
          ...
          amount={fares.find((f) => f.carId === selectedCarId)?.fare || 0}
          bookingDetails={{
            customerName: customerName.trim(),
            customerPhone: customerPhone.trim(),
            carModel:
              fares.find((f) => f.carId === selectedCarId)?.car.modelName || "",
            distanceKm,
            durationMin,
            pickup: pickupCoords,
